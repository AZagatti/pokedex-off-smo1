...
Git remote (SSH): `https://github.com/AZagatti/pokedex-off-smo1.git` (already created, empty). `git init`, set remote `origin`, commit in logical chunks, push to `main`. `paths.base` must be `/pokedex-off-smo1`. The live URL will be `https://azagatti.github.io/pokedex-off-smo1/`.
